...
def quantize_factor(factor_data,
    ...
    pd.DataFrame(factor_quantile).to_csv("data/factor_quantile.csv")
...
def compute_forward_returns(factor,
    ...
    for period in sorted(periods):
        ...
        # 计算最常见的天数差值，替代 mode
        if len(days_diffs) > 0:
            # 使用 np.bincount 来获取最常见的天数差值
            # print("days_diffs:", days_diffs)
            most_common_delta = np.bincount(days_diffs).argmax()
            # print("most_common_delta:", most_common_delta)
        else:
            # print("No days_diffs")
            most_common_delta = 0  # 如果没有值，默认为0

        delta_days = period_len.components.days - most_common_delta
...
def get_clean_factor(factor,
    ...
    # 添加微小扰动
    merged_data['factor'] += np.random.randn(len(merged_data.index)) * 1e-8
    # return merged_data

    fwdret_amount = float(len(merged_data.index))
    print("fwdret_amount:", fwdret_amount)
...
def get_clean_factor_and_forward_returns(factor,
    ...
    print("forward_returns:", forward_returns)
